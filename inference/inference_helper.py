...
def get_system_prompt(group:str, expert:bool=True, more_context:bool=True, cot:bool=False) -> str:
...
- `neither`: Dies ist die Standardkategorie im Zweifelsfall. Wähle sie, wenn die Haltung neutral, ambivalent oder unklar ist, oder wenn der Sprecher die Gruppe nur sachlich erwähnt (siehe Zentrale Anweisungen)."""
        ...
        return f"""
Du bist ein präziser Analyst für politische Sprache. Deine Aufgabe ist es, die Haltung (Stance) eines Sprechers gegenüber einer markierten Gruppe zu klassifizieren, basierend auf einem Textabschnitt.
...
def get_split_test_batch(batch:pd.DataFrame, prompt_type:str, few_shot:bool=False, shots:str=None, cot:bool=False, expert=False, more_context=False) -> list[dict]:
    """ Takes a df of to be annotated data and a prompt type and returns formatted prompts.
    ...
    for _, row in batch.iterrows():
        ...
        system_prompt = get_system_prompt(group, expert, more_context, cot)
